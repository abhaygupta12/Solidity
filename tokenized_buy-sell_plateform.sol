...
//import the ERC721 token standaed from the openzeppelin library.
import "@openzeppelin/contracts/token/ERC721/ERC721.sol";

//define contract and inherit the ERC721 token standard.
contract PaintingMarketplace is ERC721 {
    uint public paintingCounter;
    address payable public owner;   
//represent a assest(here we take painting).
    struct Painting {
        uint paintingID;
        string name;
        string description;
...
// mapping to store the paintings with the painting ID as the key.  
...
// define a function for buying a painting.   
 ...
 //define a function for getting the IDs of all unsold paintings.   
...
//return current owner of the painting with given ID.  
    function getOwnerOfPainting(uint _paintingID) public view returns (address) {
        return ownerOf(_paintingID);
    }
// return name,decr. and price of a painting with the given ID.  
